...
function MeetingStatusArea({ meeting }: { meeting: Meeting }) {
  switch (meeting.status) {
    case "Pending":
      return (
        <span className="flex items-center gap-1.5 text-xs font-semibold text-amber-600">
          <Hourglass className="h-3.5 w-3.5" /> Waiting for Approval
        </span>
      );
    case "Approved":
    ...
    case "Rejected":
      return (
        <span className="flex items-start gap-1.5 text-xs text-rose-600 max-w-xs">
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900">My Meetings</h1>
          <p className="text-slate-500 mt-1">Track the status of all your meeting requests.</p>
        </div>
        <button
          onClick={() => setActivePage("book-meeting")}
          className="flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-semibold px-4 py-2.5 rounded-lg shadow transition"
        >
          <Plus className="h-4 w-4" /> Book a Meeting
        </button>
      </div>
      ...
      {isMeetingsLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          <p className="text-slate-500">You haven't booked any meetings yet.</p>
        </div>
      ) : (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className={`shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_BADGE[meeting.status]}`}>
                    {meeting.status}
                  </span>
                </div>
